...
var provider = {
  ...
  getSenderAddr: function(){ return ADDR2; }
}
...
describe('Transaction Worker forward', function() {
  ...
  it('should error on invalid signer address.', function(done) {
    new TxWorker(provider).forward('0x1234', null, null, null).catch(function(err) {
      expect(err).to.contain('Bad Request: ');
      done();
    }).catch(done);
  });

  it('should error on invalid sig.', function(done) {
    const faultySig = { r: '0x00', s: '0x00', v: 28 };
    new TxWorker(provider).forward(ADDR1, nonceAndDest, data, faultySig).catch(function(err) {
      expect(err).to.contain('Error: ');
      done();
    }).catch(done);
  });

  it('should error on wrong signer.', function(done) {
    new TxWorker(provider).forward(ADDR2, nonceAndDest, data, sig).catch(function(err) {
      expect(err).to.contain('Unauthorized: ');
      done();
    }).catch(done);
  });

  it('should handle non existing signer in factory.', function(done) {
    sinon.stub(factory.signerToProxy, 'call').yields(null, '0x0000000000000000000000000000000000000000');

    new TxWorker(provider).forward(ADDR1, nonceAndDest, data, sig).catch(function(err) {
      expect(err).to.contain('Not Found: ');
      done();
    }).catch(done);
  });

  it('should handle abandoned proxy contract.', function(done) {
  ...
  it('should handle error in tx send.', function(done) {
    sinon.stub(factory.signerToProxy, 'call').yields(null, ADDR1);
    sinon.stub(proxy.owner, 'call').yields(null, ADDR1);
  ...
  it('should allow to send tx.', function(done) {
    ...
    new TxWorker(provider).forward(ADDR1, nonceAndDest, data, sig).then(function(rsp) {
      expect(rsp).to.eql({txHash: txHash});
